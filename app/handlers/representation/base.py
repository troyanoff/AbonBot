...
class ReprBase:
    ...
    async def create_page(
        ...
        data: BaseReprListSchema = await self.service.get_list(
            company_uuid=data['company_uuid'], offset=offset)

        if await bad_response(data, **self.request):
            return
    ...
    async def not_items(self):
        ...
        await self.request.callback.message.edit_media(
    ...
    async def __call__(
        ...
        if await bad_response(items, **self.request):
            return
        ...
        if items.total_count == 1:
            await self.one_item(items=items)
            return
    ...
    async def back_state(self):
        ...
        await self.config.back_state_caller(
            uuid=data[self.config.back_item_uuid_key], **self.request)
    ...
    async def next_state(self, item: BaseModel):
        await self.config.next_state_caller(item=item, **self.request)
