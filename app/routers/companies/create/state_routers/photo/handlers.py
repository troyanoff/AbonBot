...
router = Router()
router_state = FSMCompanyCreate.photo
...
async def done(
    ...
    new_company_dict['photo_unique_id'] = largest_photo.file_unique_id
    new_company_dict['photo_id'] = largest_photo.file_id
    ...
    await state.set_state(FSMCompanyCreate.max_hour_cancel)


@router.callback_query(
    StateFilter(router_state),
    F.data.in_(FSMCompanyCreate.photo_callbacks)
)
async def cancel(
    ...
    data = await state.get_data()
    new_company_dict = data['new_company']
    new_company_dict['photo_unique_id'] = ''
    new_company_dict['photo_id'] = ''
    await state.update_data(
        new_company=new_company_dict
    )
