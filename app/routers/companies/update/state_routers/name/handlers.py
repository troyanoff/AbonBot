...
router = Router()
router_state = FSMCompanyUpdate.name
next_state = FSMCompanyUpdate.description
...
async def start_update(
    ...
    state_handler = f'{router_state.state}:start_update'
    ...
    await message.answer(
        text=terminology_lang.terms.start_update,
        reply_markup=keyboard
    )
    await state.set_state(FSMCompanyUpdate.name)
...
async def done(
    ...
    update_company_dict['name'] = message.text
    await state.update_data(
